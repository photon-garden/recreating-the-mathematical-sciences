use crate::prelude::*;
use geo::{prelude::Contains, Polygon};
...
pub fn new(app: &App, container: &Rect, num_strokes: usize) -> HorizontalStrokes {
    ...
    let grid = Grid::new(*container, 3000, num_strokes);
...
fn load_denormalized_points(app: &App, container: &Rect) -> Vec<Point2> {
    let points = crate::svg::parse_path(app, "horizontal_boundary_backup.svg");

    let width = container.lerp_w(0.889);
    let height = container.lerp_w(0.249);

    let top_left = pt2(0.029, 0.51911).denormalize(container);
