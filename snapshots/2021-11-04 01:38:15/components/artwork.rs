...
impl Artwork {
    pub fn render(&self, params: &mut RenderParams) {
        ...
        for group_progress in ZERO_TO_ONE.subdivide(5) {
            let group_phase = group_progress.denormalize(0.0, 1.0.turns());
            for individual_progress in ZERO_TO_ONE.subdivide(9) {
                let phase_offset_amplitude_modulation = time.times(0.1).normalized_sin();
