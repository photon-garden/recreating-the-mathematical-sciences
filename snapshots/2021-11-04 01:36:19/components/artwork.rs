...
impl Artwork {
    pub fn render(&self, params: &mut RenderParams) {
        ...
        for group_progress in ZERO_TO_ONE.subdivide(4) {
            let group_phase = group_progress.denormalize(0.0, 1.0.turns());
            for individual_progress in ZERO_TO_ONE.subdivide(4) {
                // let individual_phase_offset = individual_progress.denormalize(0.9, 1.1);
                let individual_phase_offset = 0.0;
