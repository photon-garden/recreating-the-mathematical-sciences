...
fn load_denormalized_boundary_polygon(app: &App, container: &Rect) -> geo::Polygon<f32> {
    ...
    let line_string: LineString<f32> = denormalized_points
        .iter()
...
impl HorizontalStrokes {
    pub fn render(&self, params: &mut RenderParams) {
        ...
        let polygon = load_denormalized_boundary_polygon(params.app, params.container);
        ...
        draw.polyline().points(points).color(soft_black());
    }
}
