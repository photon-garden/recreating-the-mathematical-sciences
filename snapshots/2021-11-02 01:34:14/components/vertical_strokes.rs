...
pub fn new(app: &App, container: &Rect, num_strokes: usize) -> VerticalStrokes {
    ...
    let rotation_amount: f32 = 0.028.turns();
    // let rotation_amount: f32 = 0.0.turns();
    let grid = Grid::new(*container, num_strokes, 3000);
    ...
    for column in grid.columns() {
        ...
        for cell in column {
            let point = cell.rect.bottom_left().rotate(rotation_amount);
...
fn load_denormalized_points(app: &App, container: &Rect) -> Vec<Point2> {
    ...
    let height = container.lerp_w(0.1965);

    let top_left = pt2(0.055, 0.504).denormalize(container);
