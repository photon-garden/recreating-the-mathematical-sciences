...
pub fn render(params: &mut RenderParams) {
    ...
    for group in groups {
        ...
        for index_in_group in 0..group.size {
            ...
            let phase_modulation = if centered_index < 0.0 {
                time.times(phase_modulation_frequency)
                    .normalized_sin()
                    .denormalize(*phase_modulation_amplitude, center_phase)
            } else {
                time.times(phase_modulation_frequency)
                    .normalized_sin()
                    .denormalize(center_phase, *phase_modulation_amplitude)
            };
