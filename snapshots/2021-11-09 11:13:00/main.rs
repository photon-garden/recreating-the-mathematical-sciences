...
fn start(app: &App) -> Model {
    ...
    Model {
        ...
        previous_sine_direction: SineDirection::Ascending,
    }
}

fn update(app: &App, model: &mut Model, _update: Update) {
    let phase_modulation_frequency = 1.0;
    let phase_modulation_sine = app.time.times(phase_modulation_frequency).normalized_sin();

    let last_line_group_index = model.line_group_sets.len() - 1;

    let sine_direction = SineDirection::get(model.phase_modulation_sine, phase_modulation_sine);
    let sine_started_ascending = matches!(
        (model.previous_sine_direction, sine_direction),
        (SineDirection::Descending, SineDirection::Ascending)
    );

    let should_update_line_group_index = app.elapsed_frames() > 0 && sine_started_ascending;

    if should_update_line_group_index {
        if model.line_group_index < last_line_group_index {
            model.line_group_index += 1;
        } else {
            model.line_group_index = 0;
        }
    }

    model.phase_modulation_sine = phase_modulation_sine;
    model.previous_sine_direction = sine_direction;
}
