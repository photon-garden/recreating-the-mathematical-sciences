...
fn load_denormalized_boundary_polygon(app: &App, container: &Rect) -> geo::Polygon<f32> {
    ...
    let denormalized_radius = container.lerp_w(normalized_radius);
...
impl HorizontalStrokes {
    pub fn render(&self, params: &mut RenderParams) {
        ...
        let circle = Path2::regular_polygon(&center, radius, 100).denormalize_xy(params.container);
        params.draw.polyline().points(circle).color(soft_black());
